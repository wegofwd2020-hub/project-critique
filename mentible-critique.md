...
⚠️ **ESCALATED, then re-scoped on inspection — and the first version of this finding was wrong.** §5 flagged `allow_origins=["*"]`, no auth, no rate-limit as *"by-design MVP omissions… before any public URL."* **The public URL now exists, and `backend/main.py:80` still read `allow_origins=["*"]`** — the precondition was crossed without the finding being addressed.

**But an earlier draft of this correction overstated it, and the overstatement is worth recording.** That draft claimed wildcard CORS on a live backend "whose request bodies carry the user's BYOK key" let any origin drive a request against the API and reach that key. **It does not.** `allow_credentials` was never enabled, so a browser will not attach a victim's bearer token to a cross-origin request from a hostile page; the BYOK key rides in the **request body**, which the client sends deliberately; and what stops a third-party page reading a user's stored key is the **same-origin policy**, not CORS. **There was no key-exfiltration path.**

What the wildcard actually cost is **defence in depth**: any page in any browser could call the API — including the anonymous `/shelves/feed` — leaving the per-IP limiter to carry that surface alone. Real, worth closing, **not a production emergency**. Correctly a **P2 hardening item, not a P0.**

✅ **Closed 2026-07-14 (Mentible PR #304).** Replaced with an explicit allowlist (`settings.cors_allow_origins`, default `https://mambakkam.net`) plus a loopback regex for the Expo dev server; `allow_credentials` pinned `False` so it cannot be enabled by accident; the `localhost.evil.com` bypass rejected via `fullmatch` and pinned by a test.

*This error is recorded rather than quietly corrected, because it is the same failure the rest of this document indicts: a confident, plausible claim that nobody checked against the code. Committed here by the reviewer.*
...
|---|---|---|
| ~~P2~~ | ✅ **CLOSED — Mentible PR #304.** ~~Lock CORS off `*` on the live backend~~ → replaced with an explicit allowlist (`settings.cors_allow_origins`) + loopback regex; `allow_credentials` pinned `False`; `localhost.evil.com` bypass rejected and tested. **Note the re-scope:** this was briefly written up here as a P0 key-exfiltration risk. **It was not one** — no credentials, key travels in the body, same-origin policy does the protecting. It was a defence-in-depth gap, correctly P2. See Corrections | Security |
...
| ✅ | **CLOSED — Mentible PR #304.** ~~In-app help promises starter libraries that do not exist~~ (`help-content/topics.ts:319,335`). Copy now states plainly that every source is user-added and **none** are curated. **The systemic hole is still open:** add the *inverse* of the coverage gate — **every help topic must map to a feature that exists**, not merely every feature to a topic. That inverse test is what would have caught a false claim shipping to users | Documentation / Trust |
...
| ~~P1~~ | ✅ **CORS CLOSED** (allowlist, PR #304) — ~~Lock CORS off `*`… **before any public URL**~~. Rate-limiting now exists (`core/rate_limit.py`, fail-closed on the anonymous feed endpoint). **Still open:** no guard was found in `config.py` refusing the all-zeros dev master key outside development | Security |
