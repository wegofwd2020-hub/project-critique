...
### Gaps & Risks
- ❌ **SNTP client has no anti-spoofing protection whatsoever** (`collectors/ntp.py:83-124`). The socket is never `connect()`-ed; `recvfrom()`'s source address is discarded and never checked; the response's originate timestamp is never verified against the request (no anti-replay); LI/VN/Mode/stratum are never validated. Only length (`≥ 48 bytes`) and a non-zero transmit timestamp are checked. **The reference channel whose job is to catch GPS spoofing is itself trivially spoofable** by anyone who can land a UDP packet on the client's ephemeral port before the real server replies. This is the headline finding: fix before trusting the NTP cross-check.
...
1. ❌ **Harden the SNTP client** — verify the response source address and echoed originate timestamp at minimum; validate mode/stratum. Without this the independent time reference is not independent of an attacker. (`collectors/ntp.py:83-124`)
