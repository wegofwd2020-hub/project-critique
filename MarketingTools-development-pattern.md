...
A second subtractive signal: the `decks/` engine ships exactly the decks that were *needed* (three Mentible audiences that a real investor/architect/author conversation requires, one Pramana buyer deck for a named pilot — John Thomas, per `campaigns.csv`), not a generic deck framework. The brand engine generalizes (`set_brand`), but the *content* is built to the actual outreach in flight.
