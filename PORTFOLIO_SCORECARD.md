...
| **medtracker** *(personal tool)* | Operator + family — personal health ops, not a market bet | One computed refill date per medication prevents run-outs more reliably than pharmacy apps plus memory | **Demand is answered — the family uses it daily** (owner-confirmed 2026-07-14). The open test is *durability*, not adoption: exercise the backup-and-restore path end-to-end and prove nothing is lost | Family gets through a full refill cycle with no run-out and no manual fallback | Recovery fails, or the computed dates drift from reality → back to the pharmacy app | Deployed 2026-07-13 (one-day build); **in daily use on real data, owner-confirmed**. Reviewed 2026-07-14: a P0 crash and a concurrent-write data-loss bug found, fixed and deployed; CI now green. Restore path still unexercised — see the private review | Deployed — in daily use |
...
- **medtracker — retire its one irreversible risk.** The family depends on it daily, and its encryption-key/backup recovery path has still never been exercised end-to-end. The 2026-07-14 review already closed the two bugs that could corrupt or lose data in flight, and CI now guards them — this is the last one, and the only item in this portfolio whose downside is *permanent*. Run the restore drill and confirm the key is escrowed. Roughly an hour. (Specifics in the private review — `medtracker/docs/critique/`.)
