...
- **medtracker — retire its one irreversible risk.** The family depends on it daily, and the recovery path is the last thing standing between a bad day and a permanent one. It is the only item in this portfolio whose downside cannot be undone, which is why it outranks work that looks bigger. Design is written; the drill follows. Roughly an hour. (Specifics in the private review — `medtracker/docs/critique/`.)
