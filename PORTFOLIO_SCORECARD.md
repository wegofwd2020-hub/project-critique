...
| **medtracker** *(personal tool)* | Operator + family — personal health ops, not a market bet | One computed refill date per medication prevents run-outs more reliably than pharmacy apps plus memory | **Demand is answered — the family uses it daily** (owner-confirmed 2026-07-14). The durability test is **answered**: the backup-and-restore path was rebuilt and drilled end-to-end on 2026-07-14, and the backup recovers | Family gets through a full refill cycle with no run-out and no manual fallback | Recovery fails, or the computed dates drift from reality → back to the pharmacy app | Deployed 2026-07-13 (one-day build); **in daily use, owner-confirmed**. Reviewed 2026-07-14; findings fixed and deployed, CI green, restore drilled and passing | Deployed — in daily use, durability proven |
...
- **medtracker — the irreversible risk is retired.** ✅ *Done 2026-07-14.* The restore path was rebuilt (it now previews before it acts, refuses a restore that would destroy newer local data, and copies aside anything it is about to overwrite) and then **drilled end to end against the real backup**: it downloads, it decrypts, and it matches. This was the only item in the portfolio whose downside could not be undone, which is why it outranked work that looked bigger. What remains is routine key hygiene, tracked in the private repo. (Specifics — `medtracker/docs/critique/`.)
