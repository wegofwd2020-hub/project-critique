...
| [mentible-cost.md](mentible-cost.md) | Mentible | Real-world cost analysis — conventional team would have spent (~$524k US / $179k blended) |
...
- **Wire a watch mechanism for wegofwd-video.** It is now the second portfolio-level shared dependency but has no `.watch/` or in-repo CI. Mirror `wegofwd-llm`'s `.github/workflows/watch.yml` (weekly + on-push + on-consumer-pin-change) so a regression in the video seam opens a PR here, and add a `wegofwd-video-last-reviewed.txt` baseline at `233f248` (v1.0.0). Pair it with the in-repo `ruff`+`pytest` CI the critique's Top-3 asks for.
