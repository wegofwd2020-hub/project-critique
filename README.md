...
Code review and architectural critique for StudyBuddy OnDemand, Mentible, Thittam, dronePrjs, MarketingTools, Pramana, Kathai Chithiram, medtracker, atri-sangam, agastya, wegofwd-expenses, local_watch, timesheet, and the claude_memory tooling.

**Reviewed:** 2026-09-13 (v3.2 — **Pramana admitted to the critique suite (full four-lens first review v1.0)** against `main` at `f23749f`, `v0.1.0`: a compliance training & tracking platform (FastAPI · SQLAlchemy async · PostgreSQL · Alembic) whose **real product is a tamper-evident, SHA-256 hash-chained append-only audit trail** — the portfolio's **best-engineered and most honest** codebase. 14,096 src LOC / **731 tests** across 79 files, CI-verified against a real Postgres 16 + Redis 7 with `ruff` + `mypy --strict` + `bandit`; pure property-tested state machines with separation-of-duties; a candid `SECURITY.md` documenting its own un-implemented controls. Feature-complete on the core loop but **not deployed** (deploy pipeline built, gated off); launch blocked on external sign-off (a named client, legal/framework review, the open SOX video pilot), not code. Headline gaps: **migrations never run in CI** (the one mechanism carrying the product's value), **no lockfile** (ironic for a determinism-arguing product), **bandit-only** dependency scanning, multi-tenancy schema-only, and a status table reading `video ✅ complete` while **no shippable lesson exists** — the first generated one was refused by a human at the fidelity gate (the control working, documented as such). LLM/video seams are **external git-pinned** (not vendored like kathai). Cost lens (private): **~7× cheaper all-in (US)** — near the suite's floor, no calendar compression, sitting with kathai as the two judgement-dominated shared-engine siblings.)
**Prior:** 2026-09-13 v3.1 (**Kathai Chithiram admitted to the critique suite (full four-lens first review v1.0)** against `main` at `e19ca0d`, `v0.1.0`: a special-needs children's story→animation `kc` CLI — **13,580 src LOC / 735 tests** (734 pass / 1 skip, mypy `--strict` clean) — whose safety architecture (three deterministic layers + an un-bypassable human-review gate) and child-data privacy regime (pseudonymize-before-egress, ZDR-or-refuse fail-closed, per-child/per-family crypto key trees, verifiable hard-delete) are **real and tested**, and the most ethically serious codebase in the portfolio. Headline gaps: **no CI at all** — the repo fails its own `ruff` blind-except rule at `generation/generator.py:120` — **no SAST** despite special-category child data, and **content-appropriateness has no deterministic check** (the human gate is the backstop). Docs overrun code: the README's "shared `wegofwd-llm` package" is actually a **local vendored** module, ADR-026 is cited 12× but absent in-repo, and ADRs 006–013 (the "context dictionary" vision) are all Proposed/unbuilt. Cost lens (private, in `wegofwd-private-docs`): **~8× cheaper all-in (US)** — the suite's **lowest** multiplier, with **no calendar compression** — the honest signature of work dominated by safety/privacy/compliance judgement that doesn't compress. Not deployed; launch gated on a clinician, a DPO/legal review, and an ops boundary.) · 2026-09-01 v3.0 (**StudyBuddy OnDemand & Mentible re-measured against `origin/main` after ~3 months of drift** — StudyBuddy → 1,454 backend tests, RLS on 21 tenant tables, live independent-teacher subscription tier, still late-build (Epic 2 hosting blocker open); Mentible → ADR-037 SME expert-validation Studio built end-to-end, ~13k→~61k LOC, 42 ADRs, wegofwd-llm pin lag closed, but `docs/STATUS.md` 717 commits stale + Celery migration half-done; **agastya, wegofwd-expenses, local_watch, timesheet first-reviewed into the suite**) · 2026-07-18 v2.9 (**atri-sangam admitted to the critique suite** — a fixed-site GPS/PNT integrity monitor; stdlib-only core, 66 tests/90 %, headline gaps = spoofable SNTP reference channel + no runner/daemon; first product under the new public/private doc split) · 2026-07-14 v2.8 (**medtracker admitted to the critique suite (full four-lens first review v1.0)**: the portfolio's first *deployed-and-in-daily-use* product, and its only one with **no LLM at runtime** — runs at $0/month, built in a single day, the control case for what the shared-engine thesis is actually worth; because it handles personal health data in production, its four documents are held in the private `medtracker` repo and not published here) · 2026-07-01 v2.7 (**wegofwd-video admitted to the critique suite (full four-lens first review v1.0)**: the shared video-generation seam is the **second** cross-cutting shared dependency in the set, load-bearing for pramana (AI Veo path) and Kathai Chithiram (deterministic-renderer path); first review flags a provenance-integrity gap — `veo` stamps `model_verified=True` with no live call ever made — and no in-repo CI/watch yet) · 2026-06-13 v2.6 (**wegofwd-llm admitted to the watch set**, critique v1.0 — the first cross-cutting shared dependency; load-bearing for StudyBuddy_OnDemand, Mentible, and Kathai Chithiram) · 2026-06-09 v2.5 (StudyBuddy OnDemand → v1.7 on `main` @ `d50bc3e`; Mentible → v2.0 on `main` @ `40166ee` with the `wegofwd-llm` extraction itself; new four-lens sets for MarketingTools and claude_memory) · 2026-06-02 v2.4 (StudyBuddy OnDemand → critique v1.6 Authoring Studio/Epic 12; new project Mentible v1.0) · May 2026 v2.3 (all three re-measured on disk: StudyBuddy v1.5, Thittam v1.3, dronePrjs v1.1) · v2.2 (adds dronePrjs first-review; StudyBuddy v1.4; Thittam v1.2) · v2.1 (StudyBuddy visual-library wave 1+2) · April 2026 v2 (proto completion, Epic 10/11 delivery, T1 secret fix, schema injection fix, multi-tenant demo expansion)
